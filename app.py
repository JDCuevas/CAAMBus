...
def getTrolleyByPlate(plate):
    if request.method == 'GET':
        return TrolleyHandler().getTrolleyPlate(plate)
...
def getTrolleysByMileageRange(mileage_low, mileage_high):
    if request.method == 'GET':
        return TrolleyHandler().getTrolleysByMileage(mileage_low, mileage_high)
...
def getItineraryByDate(date):
    if request.method == 'GET':
        return ItineraryHandler().getItineraryByDate(date)

@app.route('/CAAMBus/itineraries/start_time/<string:start_time>', methods=['GET'])
def getItineraryByStartTime(start_time):
    if request.method == 'GET':
        return ItineraryHandler().getItineraryByStartTime(start_time)

@app.route('/CAAMBus/itineraries/end_time/<string:end_time>', methods=['GET'])
def getItineraryByEndTime(end_time):
    if request.method == 'GET':
        return ItineraryHandler().getItineraryByEndTime(end_time)

@app.route('/CAAMBus/itineraries/driver/<int:driver_id>', methods=['GET'])
def getItineraryByDriverId(driver_id):
    if request.method == 'GET':
        return ItineraryHandler().getItineraryByDriverId(driver_id)

@app.route('/CAAMBus/itineraries/trolley/<int:trolley_id>', methods=['GET'])
def getItineraryByTrolleyId(trolley_id):
    if request.method == 'GET':
        return ItineraryHandler().getItineraryByTrolleyId(trolley_id)

@app.route('/CAAMBus/itineraries/route/<int:route_id>', methods=['GET'])
def getItineraryByRouteId(route_id):
    if request.method == 'GET':
        return ItineraryHandler().getItineraryByRouteId(route_id)
